#!/usr/bin/env python3
"""
Modified Agent-Lift system using single insight agent with multiple tools.
Based on original src.py from LIft-Agent subfolder.
"""
...
file_path = os.getcwd()
prompt_file_path_agent = os.path.join(file_path, "prompts/Insight_Agent_Prompt.txt")
...
def analyze_expense_data(
    question: str, dataset: pd.DataFrame, data_description: str
) -> dict:
...
def insight_agent_workflow(
    ...
    return {
        "messages": [HumanMessage(content=message, name="Insight Agent")],
